...
const INSTRUCTIONS =
  ...
  `- If an IN THEIR OWN WORDS section is given, that's a general BASELINE the traveler set once (broad likes and ` +
  `dislikes by category) -- actual rating history is more specific and should win when the two disagree on the ` +
  `same thing. Lean on the baseline for categories no rating has touched yet.\n` +
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // so they're never sent up here as a fake ✗ -- unlike a real ✓/✗, the
    // client also deliberately leaves them OUT of passedIds (see votedIds
    // in pickFeedback.js), so an "unsure" landmark stays eligible to come
    // back up here on a future call instead of being blacklisted for good.
