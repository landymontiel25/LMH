...
// Pops up right after any successful check-in to capture a rating + selfie.
export default function CheckInReview() {
  const { justCheckedIn, pendingPhoto, celebration, clearJustCheckedIn } = useCheckIn();
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (justCheckedIn) {
      setStars(0);
      setComment('');
      // The photo taken at check-in is already attached — show it here (up to 3).
      const initial = pendingPhoto ? [pendingPhoto] : [];
      setPhotoFiles(initial);
      setPhotoPreviews(initial.map((f) => URL.createObjectURL(f)));
      setMsg(null);
    }
  }, [justCheckedIn, pendingPhoto]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      await reloadRatings();
      if (res?.photoFailed) {
        // Rating + comment saved; only the photo didn't. Don't trap the user.
        setMsg("Rating saved! Your photo couldn't upload — tap Skip to close.");
        setSaving(false);
        return;
      }
      clearJustCheckedIn();
  ...
  return (
    <div className="modal-backdrop" onClick={() => !saving && clearJustCheckedIn()}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        <h3 style={{ marginTop: 0 }}>{'\u{1F3AF}'} Checked in! +{celebration?.points ?? 100} pts</h3>
        {celebration?.message && (
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
          ...
          <button className="btn btn-ghost" onClick={clearJustCheckedIn} disabled={saving}>
            Skip
          </button>
        </div>
      </div>
