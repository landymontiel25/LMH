...
const CHECKIN_MILESTONES = [
  { n: 1, label: 'First Steps', icon: '\u{1F463}' },
  { n: 5, label: 'Explorer', icon: '\u{1F9ED}' },
  { n: 10, label: 'Adventurer', icon: '\u{26F0}\u{FE0F}' },
  { n: 25, label: 'Legend', icon: '\u{1F3C6}' },
];
const CITY_MILESTONES = [
  { n: 2, label: 'City Hopper', icon: '\u{1F306}' },
  { n: 3, label: 'Globetrotter', icon: '\u{1F30D}' },
];
const STREAK_MILESTONES = [
  { n: 3, label: '3-Day Streak', icon: '\u{1F525}' },
  { n: 7, label: '7-Day Streak', icon: '\u{1F525}' },
  { n: 30, label: '30-Day Streak', icon: '\u{1F525}' },
];

export function computeBadges({ checkinsCount, citiesCount, streakDays }) {
  const badges = [];
  for (const m of CHECKIN_MILESTONES) if (checkinsCount >= m.n) badges.push({ id: `checkins-${m.n}`, label: m.label, icon: m.icon });
  for (const m of CITY_MILESTONES) if (citiesCount >= m.n) badges.push({ id: `cities-${m.n}`, label: m.label, icon: m.icon });
  for (const m of STREAK_MILESTONES) if (streakDays >= m.n) badges.push({ id: `streak-${m.n}`, label: m.label, icon: m.icon });
  return badges;
}
